...
- **被弾表タブ**: 登録済みデータから各武将の行動状況（未行動 / 行動可 / 行動済み / 兵力減）を集計表示。守備敗北から40分未満は兵力減、40分経過後は行動可として扱い、壁戦も勝敗にかかわらず出兵として判定します。武将名での絞り込みと時刻の自動更新にも対応します
...
- **管理Wikiタブ**: 管理者向けの運用手順・判断基準を複数ページのMarkdownで作成・編集。編集 / プレビュー / 分割表示、タイトル検索、未保存変更の警告、同時更新の競合検知、`Cmd`/`Ctrl` + `S` での保存に対応し、閲覧を含む全操作を管理者だけに限定
