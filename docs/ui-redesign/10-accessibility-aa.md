...
UI 10ではテーマをまたぐコントラストを優先して国色を文字へ直接使用しない方針としたが、後続変更で国名の識別性と環境設定の再現性を優先し、設定済みの国カラーを国名の文字色へそのまま反映する仕様へ変更した。

- 設定色をテーマ本文色と混ぜる処理を行わない
- 国名リンクのHover / Focusでも設定色の明度を変更しない
- 国名以外の本文色、国色の設定値、保存APIは変更しない
- 戦闘履歴の枠・背景に使う国色のチントは維持する
- 白・黄・黒など背景とのコントラストが不足する設定色では、設定した利用者による確認が必要になる
